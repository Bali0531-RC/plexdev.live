...
server {
    ...
    # SSL configuration will be added by Certbot below
...
<!DOCTYPE html><html><head><title>PlexStore - Service Unavailable</title><style>body{font-family:Arial,sans-serif;text-align:center;padding:50px;}h1{color:#e74c3c;}p{font-size:18px;}</style></head><body><h1>502 - Service Unavailable</h1><p>The PlexStore service is currently starting up or unavailable. Please try again shortly.</p><p>If this issue persists, please check the service status or contact the administrator.</p></body></html>
EOF
        # Ownership should be root:root now
...
setup_ssl() {
    ...
    if sudo certbot certificates | grep -q "Domains: $domain"; then
        ...
        # Get SSL certificate for the first time
        # --redirect automatically handles HTTP->HTTPS redirection in Nginx
        # --keep-until-expiring prevents unnecessary renewals if cert is still valid
        sudo certbot --nginx -d "$domain" --non-interactive --agree-tos --email "$email" --redirect --keep-until-expiring
    fi


    if [ $? -eq 0 ]; then
        print_success "SSL certificate obtained/updated successfully for $domain."
...
[Service]
Type=simple
User=root  # Changed from $PLEX_USER
Group=root # Changed from $PLEX_GROUP
WorkingDirectory=$install_path
ExecStart=$NODE_EXECUTABLE .

# Environment variables can be set here if needed
# Environment="NODE_ENV=production"
# Environment="PORT=3000" # Example, though config.yml is usually preferred

Restart=on-failure
RestartSec=10 # Restart delay
TimeoutStartSec=30s # Time to allow for startup
StandardOutput=syslog # Log stdout to syslog
StandardError=syslog # Log stderr to syslog
SyslogIdentifier=$service_name

# Security settings (Consider keeping some even when running as root)
# NoNewPrivileges=true # Recommended
# ProtectSystem=strict # More restrictive than 'full'
# ProtectHome=true
# PrivateTmp=true
# PrivateDevices=true
